...
public:
    ...
    void displayDueTasks() {
        ...
        cout << "\t\t\tCurrent Date: " << (current->tm_year + 1900) << '-'
             << (current->tm_mon + 1) << '-' << current->tm_mday << endl;
        ...
        for (const Task& task : tasks) {
            ...
            if (dueDate.tm_year < current->tm_year ||
                (dueDate.tm_year == current->tm_year && dueDate.tm_mon < current->tm_mon) ||
                (dueDate.tm_year == current->tm_year && dueDate.tm_mon == current->tm_mon && dueDate.tm_mday < current->tm_mday)) {
                cout<<endl<<endl<<endl;
...
int main() {
    ...
    cout << "\t\t\t\t";
    printLine('>', 50);
    printWithDelay("\t\t\t\t\t      WELCOME TO TO DO LIST      \n", 100);
    cout << "\t\t\t\t";
    printLine('>', 50);
    cout<<"\n\n\n";
    printWithDelay( "\t\t\t\tUnleash the power of productivity with our\n"
            "\t\t\t\tcutting-edge To Do List. Seamlessly manage\n"
